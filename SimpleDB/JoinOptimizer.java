...
public class JoinOptimizer {
    ...
    private static double RANGE_SELECTIVITY = 0.35;
    /**
     * Constructor
     * 
     * @param p
    ...
    public double estimateJoinCost(LogicalJoinNode j, int card1, int card2,
	    ...
	    double d = cost1 + card1*cost2 + card1*card2;
            return d;
        }
    }
	...
	for(int i = 1; i <= joins.size(); i++){
		...
		for(LogicalJoinNode jn: joinSet){
		    CostCard subplan = computeCostAndCardOfSubplan(stats, filterSelectivities, jn, joinSet, bestPlan.cost, planCache);
		    if(subplan != null){
        ...
        Set<LogicalJoinNode> orderedJoins = new HashSet<LogicalJoinNode>();
	orderedJoins.addAll(joins);

	if (explain) {
	    printJoins(planCache.getOrder(orderedJoins), planCache, stats, filterSelectivities);
	}

        return planCache.getOrder(orderedJoins);
    }
